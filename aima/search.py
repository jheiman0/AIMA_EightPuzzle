...
def best_first_graph_search(problem, f, display=False):
    ...
    while frontier:
        ...
        if problem.goal_test(node.state):
            if display:
                file.write('{} paths have been expanded and {} paths remain in the frontier'.format(len(explored), len(frontier)))
                file.close()
            return node
        explored.add(node.state)
        for child in node.expand(problem):
...
class EightPuzzle(Problem):
    ...
    # ECE group implementation
    def manhatten(self, node):
        return sum(
            abs((i % 3) - (j % 3)) + abs((i // 3) - (j // 3))
            for i, j in ((node.state.index(k), self.goal.index(k)) for k in range(1, 9))
        )

    # ECE group implementation
    def nillson(self, node):
        score = self.manhatten(node)
        for i in range(len(self.goal)-1):
            if node.state.index(self.goal[i]) == 8:
                score += 3
            elif node.state.index(self.goal[i+1]) != (node.state.index(self.goal[i]) + 1) % 8:
                score += 6
        return score
